/**
 * 设计提醒：此场景以路径动画与单一开始按钮组织节奏；点击后只下载固定白名单文本，不显示桌面、文件夹或预览页面。
...
export function RansomwareDesktopScenario({ onLeave, onReset, onStageAudio, voiceEnabled }: RansomwareDesktopScenarioProps) {
  ...
  const downloadConfiguredFile = () => {
    ...
    if (!assetUrl.startsWith("/manus-storage/") || !isAllowedRansomwareDesktopFile(downloadName)) return;
    void fetch(assetUrl)
      .then((response) => (response.ok ? response.blob() : Promise.reject(new Error("资料无法获取"))))
      .then((blob) => {
  ...
  return (
    <div className="relative z-10 min-h-screen overflow-hidden bg-[#102b47] text-white">
      ...
      <main className="relative min-h-[calc(100vh-40px)] overflow-hidden bg-[radial-gradient(ellipse_at_76%_14%,rgba(124,217,246,.34),transparent_21%),radial-gradient(circle_at_16%_76%,rgba(67,154,215,.24),transparent_31%),linear-gradient(136deg,#0e3156_0%,#15578a_48%,#102b47_100%)] px-5 py-7 sm:px-10 sm:py-10">
        ...
        {phase === "ready" && <section className="relative z-10 grid min-h-[calc(100vh-150px)] place-items-center"><button type="button" onClick={downloadConfiguredFile} className="inline-flex min-w-36 items-center justify-center rounded-lg bg-[#e6c35b] px-7 py-3.5 text-base font-semibold text-[#19314a] shadow-[0_10px_24px_rgba(8,19,35,.25)] transition hover:bg-[#f5d47a] active:scale-[.98]">开始</button></section>}
